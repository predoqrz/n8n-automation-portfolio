...
**Integrações:** Gmail · OpenAI · Trello · PostgreSQL
...
no quadro do time. É trabalho repetitivo, acontece dezenas de vezes por dia e atrasa
...
cria o cartão automaticamente quando o próprio modelo declara confiança suficiente —
o resto vai para uma lista de revisão humana, nunca é descartado.
...
flowchart LR
    ...
    F --> G{Confiança ≥ 0,7?}
    G -->|sim| H[Rotear para lista da categoria]
    G -->|não| I[Encaminhar para revisão manual]
    E -.erro.-> I
    H --> J[Chamado triado]
    I --> J
    J --> K[Criar cartão no Trello]
    K --> L[Montar registro de log]
    K -.erro.-> M[Registrar falha do Trello]
...
| 2  | Normalizar campos do chamado | Edit Fields (Set) | Reduz o e-mail a seis campos estáveis: `mensagem_id`, `thread_id`, `remetente`, `assunto`, `corpo`, `recebido_em`. Limpa HTML e corta o corpo em 4000 caracteres. |
...
| 9  | Rotear para lista da categoria | Edit Fields (Set) | Mapa `categoria → $env.TRELLO_LIST_*`. |
...
| 12 | Criar cartão no Trello | Trello | Cria o cartão na lista resolvida. 3 tentativas, saída de erro ligada. |
...
| 16 | Registrar falha do Trello | Edit Fields (Set) | Caminho de erro do Trello: registra `destino: falha-trello` e **não** marca o e-mail. |
...
- **Por que um mapa `categoria → lista` e não um Switch com quatro saídas:** o Switch
  exigiria quatro nós Trello idênticos. Acrescentar a quinta categoria seria acrescentar um
  ramo inteiro; com o mapa, é uma linha na expressão e uma variável no `.env`.

- **Por que dois mecanismos de idempotência:** a label do Gmail sozinha tem uma janela de
  falha — se o cartão é criado e a chamada de label falha, o próximo ciclo cria um segundo
  cartão. Por isso o log no Postgres é gravado **antes** da label, e `mensagem_id` tem
  constraint `UNIQUE`. A label é conveniência; o banco é a garantia.

- **Por que `Revisão manual` é uma lista do Trello e não um e-mail para o analista:** o
  chamado precisa ficar no mesmo lugar onde o time já trabalha. Chamado que sai do quadro
  vira chamado esquecido.
...
|---|---|
| API do LLM fora, timeout ou JSON irrecuperável | 2 tentativas; depois a saída de erro manda para revisão manual com `motivo` preenchido. O e-mail vira cartão do mesmo jeito. |
...
| Trello fora do ar | 3 tentativas com 3s de espera; depois grava `destino: falha-trello` no log e **não** marca o e-mail — ele volta na próxima execução. |
...
Um board com cinco listas: `Acesso`, `Rede`, `Equipamento`, `Sistemas` e `Revisão manual`.
Os IDs saem de `https://trello.com/b/SEU_BOARD.json`, no array `lists`.
...
Gmail (OAuth2), OpenAI (API key), Trello (API key + token) e Postgres
...
e espere o próximo ciclo. O fluxo deve parar em `Ignorar duplicado` e **não** criar um
segundo cartão.

**Caso 4 — falha do LLM.** Desligue a credencial da OpenAI e mande um e-mail. Deve virar
cartão em `Revisão manual` com o `motivo` mostrando o erro.
...
- [ ] Responder o remetente com o link do cartão, para ele saber que o chamado foi aberto.
